...
def print_info():
    ...
    if keras.backend._BACKEND == "tensorflow":
        ...
        print(
            "Keras backend: {}: {}".format(
                keras.backend._backend, tensorflow.__version__
            )
        )
        ...
        print(
            "Keras backend: {}: {}".format(keras.backend._backend, theano.__version__)
        )
...
def check_model(model):
    ...
    model.fit(
        np.random.uniform(size=batch_input_shape),
        np.random.uniform(size=batch_output_shape),
        epochs=1,
    )
...
def visualise_model(model, logam=False, sr=16000):
    ...
    src, _ = librosa.load(
        "/Users/admin/Dropbox/workspace/unet/data/audio/abjones_1_01.wav", sr, mono=True
    )
    src = src[:nsp_src]
...
def test_plot():
    ...
    display.specshow(
        librosa.amplitude_to_db(
            np.abs(librosa.stft(src_cute[: SR * 3], 512, 256)) ** 2, ref=1.0
        ),
        y_axis="linear",
        sr=SR,
    )
